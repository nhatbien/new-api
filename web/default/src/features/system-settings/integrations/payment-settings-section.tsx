...
const paymentSchema = z.object({
  ...
  SepayAccountNumber: z.string(),
  SepayAccountName: z.string(),
  SepayWebhookSecret: z.string(),
  SepayQrTemplate: z.string(),
...
export function PaymentSettingsSection({
  ...
  const saveSepaySettings = async () => {
    const values = form.getValues()
    const sanitized = {
      SepayEnabled: values.SepayEnabled as boolean,
      SepayBankCode: values.SepayBankCode.trim(),
      SepayAccountNumber: values.SepayAccountNumber.trim(),
      SepayAccountName: values.SepayAccountName.trim(),
      SepayWebhookSecret: values.SepayWebhookSecret.trim(),
      SepayQrTemplate: values.SepayQrTemplate.trim() || 'compact2',
      SepayMinTopUp: values.SepayMinTopUp as number,
    }

    const initial = {
      SepayEnabled: initialRef.current.SepayEnabled,
      SepayBankCode: initialRef.current.SepayBankCode.trim(),
      SepayAccountNumber: initialRef.current.SepayAccountNumber.trim(),
      SepayAccountName: initialRef.current.SepayAccountName.trim(),
      SepayWebhookSecret: initialRef.current.SepayWebhookSecret.trim(),
      SepayQrTemplate: initialRef.current.SepayQrTemplate.trim() || 'compact2',
    ...
    Object.entries(sanitized).forEach(([key, value]) => {
      if (
        ...
        (!value || value === initial.SepayWebhookSecret)
      ) {
        return
      }
    ...
    >
      ...
      <Form {...form}>
        ...
        >
          ...
          <div className='space-y-4'>
            <div>
              <h3 className='text-lg font-medium'>{t('SEPAY VietQR')}</h3>
              <p className='text-muted-foreground text-sm'>
                {t('Accept bank transfers through SEPAY and VietQR.io')}
              </p>
            </div>
              ...
              <ul className='list-inside list-disc space-y-1'>
                <li>
                  ...
                  <code className='rounded bg-green-100 px-1 py-0.5 text-xs dark:bg-green-900'>
                    {'<ServerAddress>/api/sepay/webhook'}
                  </code>
                </li>
                <li>
                  {t('Use the webhook secret as Authorization Bearer token')}
                </li>
              </ul>
            </div>
              ...
              render={({ field }) => (
                <FormItem className='flex items-center justify-between rounded-lg border p-3'>
                  <div className='space-y-0.5'>
                    <FormLabel>{t('Enable SEPAY VietQR')}</FormLabel>
                    <FormDescription>
                      {t('Users will scan a VietQR.io QR code to pay')}
                    </FormDescription>
                  </div>
                  <FormControl>
                    <Switch
            ...
            <div className='grid gap-6 md:grid-cols-3'>
              ...
              <FormField
                control={form.control}
                name='SepayQrTemplate'
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('VietQR template')}</FormLabel>
                    <FormControl>
                      <Input placeholder='compact2' {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
