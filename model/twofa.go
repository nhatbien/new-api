...
func (t *TwoFA) ValidateTOTPAndUpdateUsage(code string) (bool, error) {
	// 检查是否被锁定
	if t.IsLocked() {
		return false, fmt.Errorf("the account is locked, please try again after %v", t.LockedUntil.Format("2006-01-02 15:04:05"))
	}
	...
	if !common.ValidateTOTPCode(t.Secret, code) {
		...
		if err := t.IncrementFailedAttempts(); err != nil {
			common.SysLog("failed to update 2FA failed-attempt count: " + err.Error())
		}
		return false, nil
	}
