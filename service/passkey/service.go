...
autoDetect:
	...
	if scheme == "http" && !settings.AllowInsecureOrigin && r.Host != "localhost" && r.Host != "127.0.0.1" && !strings.HasPrefix(r.Host, "127.0.0.1:") && !strings.HasPrefix(r.Host, "localhost:") {
		return nil, fmt.Errorf("Passkey only supports HTTPS; current access is %s://%s. Please allow insecure Origin in Passkey settings or configure HTTPS", scheme, r.Host)
	}
	// 优先使用请求的完整Host（包含端口）
	host := r.Host
...
func resolveRPID(r *http.Request, settings *system_setting.PasskeySettings, origins []string) (string, error) {
	...
	if len(origins) == 0 {
		return "", errors.New("Passkey Origin is not configured, unable to derive RPID")
	}
