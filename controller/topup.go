...
func getPayMoney(amount int64, group string, applyDiscount bool) float64 {
...
func RequestEpay(c *gin.Context) {
	...
	payMoney := getPayMoney(req.Amount, group, true)
	if payMoney < 0.01 {
	...
	if !operation_setting.ContainsPayMethod(req.PaymentMethod) {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "payment method not found"})
		return
	}
	...
	if client == nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "payment information is not configured by the current administrator"})
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Epay failed to initiate payment user_id=%d trade_no=%s payment_method=%s amount=%d error=%q", id, tradeNo, req.PaymentMethod, req.Amount, err.Error()))
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Epay top-up order created successfully user_id=%d trade_no=%s payment_method=%s amount=%d money=%.2f uri=%q params=%q", id, tradeNo, req.PaymentMethod, req.Amount, payMoney, uri, common.GetJsonString(params)))
...
func EpayNotify(c *gin.Context) {
	...
	if len(params) == 0 {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Epay webhook received empty parameters path=%q client_ip=%s", c.Request.RequestURI, c.ClientIP()))
		_, _ = c.Writer.Write([]byte("fail"))
		return
	}
	client := GetEpayClient()
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.PaymentProvider != model.PaymentProviderEpay {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("Epay order payment gateway mismatch trade_no=%s order_provider=%s callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, topUp.PaymentProvider, verifyInfo.Type, c.ClientIP()))
			return
		}
		if topUp.Status == common.TopUpStatusPending {
			...
			model.RecordTopupLog(topUp.UserId, fmt.Sprintf("Top-up via online payment succeeded, top-up amount: %v, payment amount: %f", logger.LogQuota(quotaToAdd), topUp.Money), c.ClientIP(), topUp.PaymentMethod, "epay")
		}
	} else {
...
func RequestAmount(c *gin.Context) {
	...
	payMoney := getPayMoney(req.Amount, group, false)
	if payMoney <= 0.01 {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "top-up amount is too low"})
		return
	}
