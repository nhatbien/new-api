...
func PasskeyRegisterBegin(c *gin.Context) {
	if !system_setting.GetPasskeySettings().Enabled {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "Passkey login is not enabled by the administrator",
		})
		return
	}
...
func PasskeyLoginFinish(c *gin.Context) {
	...
	if !ok {
		common.ApiErrorMsg(c, "Passkey login state is abnormal")
		return
	}
	...
	if updatedCredential == nil {
		common.ApiErrorMsg(c, "failed to update Passkey credential")
		return
	}
	now := time.Now()
...
func AdminResetPasskey(c *gin.Context) {
	...
	if _, err := model.GetPasskeyByUserID(user.Id); err != nil {
		if errors.Is(err, model.ErrPasskeyNotFound) {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "this user has not bound a Passkey yet",
			})
			return
		}
		common.ApiError(c, err)
...
func PasskeyVerifyFinish(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "Passkey verification succeeded",
	})
}
