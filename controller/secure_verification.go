...
func UniversalVerify(c *gin.Context) {
	...
	if !has2FA && !hasPasskey {
		common.ApiError(c, fmt.Errorf("the user has not enabled 2FA or Passkey"))
		return
	}
	...
	case "2fa":
		if !has2FA {
			common.ApiError(c, fmt.Errorf("the user has not enabled 2FA"))
			return
		}
		if req.Code == "" {
	...
	case "passkey":
		if !hasPasskey {
			common.ApiError(c, fmt.Errorf("the user has not enabled Passkey"))
			return
		}
		// Passkey branch only trusts the short-lived marker written by PasskeyVerifyFinish.
		verified, err = consumePasskeyReady(c)
		if err != nil {
			common.ApiError(c, fmt.Errorf("invalid Passkey verification state: %v", err))
			return
		}
		if !verified {
	...
	if !verified {
		common.ApiError(c, fmt.Errorf("verification failed, please check the code"))
		return
	}
	...
	// 记录日志
	model.RecordLog(userId, model.LogTypeSystem, fmt.Sprintf("Universal secure verification succeeded (method: %s)", verifyMethod))
