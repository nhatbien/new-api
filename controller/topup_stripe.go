...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	if req.SuccessURL != "" && common.ValidateRedirectURL(req.SuccessURL) != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": "Success redirect URL is not in the trusted domain list", "data": ""})
		return
	}

	if req.CancelURL != "" && common.ValidateRedirectURL(req.CancelURL) != nil {
		c.JSON(http.StatusBadRequest, gin.H{"message": "Cancel redirect URL is not in the trusted domain list", "data": ""})
		return
	}
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Stripe top-up order created successfully user_id=%d trade_no=%s amount=%d money=%.2f", id, referenceId, req.Amount, chargedMoney))
...
func sessionCompleted(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if "complete" != status {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe checkout.completed has invalid status, skipping trade_no=%s status=%s client_ip=%s", referenceId, status, callerIp))
		return
	}

	paymentStatus := event.GetObjectValue("payment_status")
	if paymentStatus != "paid" {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe checkout payment not completed yet, waiting for async result trade_no=%s payment_status=%s client_ip=%s", referenceId, paymentStatus, callerIp))
		return
	}
...
func sessionAsyncPaymentFailed(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if topUp.Status != common.TopUpStatusPending {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe async payment failed but order is not pending, skipping trade_no=%s status=%s client_ip=%s", referenceId, topUp.Status, callerIp))
		return
	}
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) {
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe missing order number while completing order client_ip=%s", callerIp))
		return
	}
...
func sessionExpired(ctx context.Context, event stripe.Event) {
	...
	status := event.GetObjectValue("status")
	if "expired" != status {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe checkout.expired has invalid status, skipping trade_no=%s status=%s", referenceId, status))
		return
	}
	...
	if err := model.ExpireSubscriptionOrder(referenceId, model.PaymentProviderStripe); err == nil {
		...
		logger.LogError(ctx, fmt.Sprintf("Stripe subscription order expiration handling failed trade_no=%s error=%q", referenceId, err.Error()))
		return
	}

	err := model.UpdatePendingTopUpStatus(referenceId, model.PaymentProviderStripe, common.TopUpStatusExpired)
	if errors.Is(err, model.ErrTopUpNotFound) {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe top-up order not found, cannot mark expired trade_no=%s", referenceId))
		return
	}
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("Stripe top-up order expiration handling failed trade_no=%s error=%q", referenceId, err.Error()))
		return
	}
...
func getStripePayMoney(amount float64, group string) float64 {
	...
	discount := operation_setting.GetAmountDiscount(int(originalAmount))
