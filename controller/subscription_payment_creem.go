...
func SubscriptionRequestCreemPay(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Failed to read Creem subscription payment request error=%q", err.Error()))
	...
	if plan.CreemProductId == "" {
		common.ApiErrorMsg(c, "This plan has no CreemProductId configured")
		return
	}
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Failed to create Creem subscription payment link trade_no=%s product_id=%s error=%q", referenceId, product.ProductId, err.Error()))
