...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		logger.LogWarn(context.Background(), fmt.Sprintf("Creem webhook secret is not configured test_mode=%t signature=%q body=%q", setting.CreemTestMode, signature, payload))
...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Invalid product configuration"})
		return
	}
	...
	if selectedProduct == nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Product not found"})
		return
	}
...
func RequestCreemPay(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Request read failed"})
		return
	}
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook received request path=%q client_ip=%s signature=%q body=%q", c.Request.RequestURI, c.ClientIP(), signature, string(bodyBytes)))
	if signature == "" {
	...
	if err := c.ShouldBindJSON(&webhookEvent); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook parsing failed path=%q client_ip=%s error=%q body=%q", c.Request.RequestURI, c.ClientIP(), err.Error(), string(bodyBytes)))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	...
	default:
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook ignored event event_type=%s event_id=%s", webhookEvent.EventType, webhookEvent.Id))
...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	if event.Object.Order.Status != "paid" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem order is not paid, skipping request_id=%s order_id=%s order_status=%s", event.Object.RequestId, event.Object.Order.Id, event.Object.Order.Status))
		c.Status(http.StatusOK)
		return
	}
	...
	if event.Object.Order.Type != "onetime" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem order type is not supported yet, skipping request_id=%s creem_order_id=%s order_type=%s", referenceId, event.Object.Order.Id, event.Object.Order.Type))
		c.Status(http.StatusOK)
		return
	}
	...
	if topUp.Status != common.TopUpStatusPending {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem top-up order is not pending, skipping trade_no=%s status=%s creem_order_id=%s", referenceId, topUp.Status, event.Object.Order.Id))
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem top-up succeeded trade_no=%s creem_order_id=%s quota=%d money=%.2f client_ip=%s", referenceId, event.Object.Order.Id, topUp.Amount, topUp.Money, c.ClientIP()))
	c.Status(http.StatusOK)
}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	if checkoutResp.CheckoutUrl == "" {
		return "", fmt.Errorf("Creem API response has no checkout URL")
	}
