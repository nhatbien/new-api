...
func buildUpstreamModelUpdateTaskNotificationContent(
	...
	builder.WriteString(fmt.Sprintf(
		"Upstream model audit summary: checked %d channels, detected %d changes, added %d, removed %d, auto-synced additions %d, failed %d.",
		checkedChannels,
		changedChannels,
		detectedAddModels,
	...
	if len(channelSummaries) > 0 {
		...
		if len(channelSummaries) > displayCount {
			builder.WriteString(fmt.Sprintf("\n- The remaining %d channels have been omitted", len(channelSummaries)-displayCount))
		}
	}
	...
	if len(normalizedAddModelSamples) > 0 {
		...
		if len(normalizedAddModelSamples) > displayCount {
			builder.WriteString(fmt.Sprintf(" (the remaining %d have been omitted)", len(normalizedAddModelSamples)-displayCount))
		}
	}
	...
	if len(normalizedRemoveModelSamples) > 0 {
		...
		if len(normalizedRemoveModelSamples) > displayCount {
			builder.WriteString(fmt.Sprintf(" (the remaining %d have been omitted)", len(normalizedRemoveModelSamples)-displayCount))
		}
	}
	...
	if failedChannels > 0 {
		...
		if failedChannels > displayCount {
			builder.WriteString(fmt.Sprintf(" (the remaining %d have been omitted)", failedChannels-displayCount))
		}
	}
...
func runChannelUpstreamModelUpdateTaskOnce() {
	...
	if changedChannels > 0 || failedChannels > 0 {
		...
		service.NotifyUpstreamModelUpdateWatchers(
			"Upstream model audit notification",
